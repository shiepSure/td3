...
from rand_bat_gen import ESSEnv
import pandas as pd
...
if __name__ == '__main__':
    ...
    if args.test_network:
        ...
        eval_data.columns = ['time_step', 'price', 'netload', 'action', 'real_action', 'soc', 'ev', 'gen1', 'gen2',
    ...
    if args.compare_with_pyomo:
        base_result = optimization_base_result(env, month, day, initial_soc)
        eval_cost = sum(eval_data['operation_cost'])
        opt_cost = sum(base_result['step_cost'])
        print(f"RL Cost: {eval_cost}, Optimization Cost: {opt_cost}")
        if eval_cost > opt_cost:
            print("需要进一步优化RL策略")